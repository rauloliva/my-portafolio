...
const Layout = props => {
    const color_layout = props.theme == 'dark' ? style.layout_dark : style.layout_light
    ...
    return (
        <div className={style.layout.concat(' ', color_layout)}>
            <nav className={style.layout__nav}>
                ...
                <Image src={change_contrast_img} onClick={() => props.changeTheme(props.theme)} alt='Moon' className={style.layout__nav_change_contrast} />
            </nav>
            {props.children}
        </div>
    )
